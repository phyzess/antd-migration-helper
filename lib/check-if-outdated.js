...
function checkNodeVersion () {
  ...
  if (minNodeVersion > currentNodeVersion) {
    console.log(chalk.red(
      'You must upgrade node to >=' + minNodeVersion + ' to use antd-migration-helper\n'
    ))
    process.exit(1)
  }
}
...
function checkMigrationHelperVersion (done) {
  request({
    url: 'https://registry.npmjs.org/antd-migration-helper',
    timeout: 1000
    ...
    if (!error && response.statusCode === 200) {
      ...
      if (semver.lt(localVersion, latestVersion)) {
        console.log()
        console.log(chalk.yellow('A newer version of antd-migration-helper is available.'))
        console.log()
        ...
        console.log(chalk.green(
          '  npm update --global antd-migration-helper'
        ))
        process.exit(1)
      }
    }
